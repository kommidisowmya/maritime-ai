...
function drawChart(psnr, ssim, uiqm) {
  ...
  // Initial dataset with 0 values
  const data = {
    labels: ["PSNR", "SSIM(x100)", "UIQM(x10)"],
    datasets: [{
      data: [0, 0, 0],
  ...
  const finalValues = [psnr, ssim * 100, uiqm * 10];
  const steps = 60; // number of animation steps
  let currentStep = 0;

  const interval = setInterval(() => {
    currentStep++;
    chart.data.datasets[0].data = finalValues.map((v) => (v / steps) * currentStep);
    chart.update();
    if (currentStep >= steps) clearInterval(interval);
  }, 15); // update every 15ms (~900ms total)
}
